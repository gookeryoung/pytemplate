...
from {{ package_name }} import RequestBatch
...
from {{ package_name }} import validate_non_empty
...
from {{ package_name }} import format_size
...
from {{ package_name }} import services
...
def test_load_config_reads_env(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("{{ package_name | upper }}_TIMEOUT", "60")
...
def test_load_config_falls_back_when_env_missing(monkeypatch: pytest.MonkeyPatch) -> None:
    """环境变量缺失时应回退到默认值。"""
    monkeypatch.delenv("{{ package_name | upper }}_TIMEOUT", raising=False)
    cfg = config.load_from_env()
...
from {{ package_name }} import discover_configs
...
from {{ package_name }} import parse_number
...
from {{ package_name }} import StorageError, load
...
# 全量（含慢测试）
uv run pytest --cov={{ package_name }} --cov-branch --cov-report=term-missing
...
from {{ package_name }}.windows.main_window import MainWindow
...
from {{ package_name }}.widgets.search_box import SearchBox
