...
| pytest-qt | `>=4.2.0`，Qt 测试框架（qapp/qtbot fixture） |
| PyInstaller | `>=6.0`，打包为可执行文件 |
| pyside2-rcc | 资源编译器（.qrc → _rc.py） |
| pyside2-uic | UI 编译器（.ui → .py） |
...
- **PySide2 仅支持 Python 3.6-3.10**：PyPI 官方 wheel 最新 `5.15.2.1`，无 Python 3.11+ wheel。
  生成项目时应将 `max_python_version` 设为 `3.10`，否则 `uv sync` 在高版本 Python 上装不上 PySide2。
- **环境标记**：依赖声明为 `PySide2>=5.15.2.1; python_version <= "3.10"`，防止 3.11+ 环境解析失败。
- **PySide2 vs PySide6 API 差异**：`app.exec_()`（带下划线）vs PySide6 的 `app.exec()`；`QCoreApplication.exec_` 同理。
  本模板代码统一用 `exec_()`。
- **import 路径**：`from PySide2.QtWidgets import QApplication, QLabel`，模块名区分大小写。
- 升级到 PySide6 时需替换 `exec_`→`exec`、枚举 `Qt.AlignCenter`→`Qt.AlignmentFlag.AlignCenter` 等。

## 模块与入口

- 入口 `src/<package>/main.py`：`QApplication(sys.argv)` → 构建主窗口 → `app.exec_()`。
- `main()` 加 `# pragma: no cover`（事件循环阻塞，难自动化），拆出 `create_main_window()` 等可测函数。
- 业务逻辑放纯 Python 模块（不 import PySide2），便于单测；GUI 层只做信号槽连接与状态展示。
...
```bash
pyside2-rcc resources.qrc -o src/<package>/resources_rc.py
```

- 引用资源用 `:/` 前缀：`QIcon(":/icons/app.png")`，路径前缀在 `.qrc` 的 `<qresource prefix="/">` 定义。
- `.ui` 文件可用 `pyside2-uic` 编译或运行时 `QUiLoader` 加载；编译方式更快，推荐。
- 资源变更后须重新编译 `_rc.py`，加入版本控制（避免构建环境缺 pyside2-rcc）。
- 大文件（视频/字体）不进 `.qrc`，用磁盘路径加载，避免二进制膨胀。

## 事件循环

- `QApplication` 全局单例，`sys.argv` 传入（解析 Qt 命令行参数如 `-style`）。
- `app.exec_()` 阻塞直至窗口关闭或 `app.quit()`。
...
- `qapp` fixture 提供单例 QApplication（pytest-qt 自动管理生命周期）。
...
- Nuitka 替代方案：`nuitka --standalone --enable-plugin=pyside2 main.py`，启动更快但编译慢。
...
- `.ts` 翻译文件用 `pyside2-lupdate` 提取，`pyside2-lrelease` 编译为 `.qm`。
...
- `.qrc`/`.ui`/`.qss`/`.ts` 源文件提交，编译产物 `_rc.py`/`_ui.py`/`.qm` 提交（构建环境可能无 pyside2 工具链）。
