...
- **优先标准库**：优先使用标准库实现功能；需 `typing-extensions` 用于 `override`/`TypeVar` 前向兼容
  （`python_version < '3.13'` 时自动引入）。新增第三方依赖须审慎，优先用标准库。
...
- **抽象基类**：接口用 `abc.ABC` + `@abstractmethod`。
- **枚举**：状态/标志值用 `enum.Enum`，禁止裸字符串/魔术数字；枚举值用 `UPPER_SNAKE`。
- **`__repr__`**：可变类实现 `__repr__`（含关键字段）；`frozen=True` dataclass 自动生成。

## 模块与导入

- **单一职责**：每模块只做一件事。禁止跨职责边界。
...
- **信号量限流**：按组限流时用 `Semaphore`。

## 测试

- **覆盖率 ≥ 95%**（branch coverage），不得下降。
- **公共 API 优先测试**：用公共接口测试，不访问私有方法；
...
- **统一校验**：入口校验一次，下游路径不重复。
- **事件 emit**：生命周期必须 emit 完整状态转换，不要留死分支
  （`# pragma: no cover` 是清理信号，应激活或删除）。
