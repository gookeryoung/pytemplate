...
from coopie import RequestBatch
...
from coopie import validate_non_empty
...
from coopie import format_size
...
def test_load_config_falls_back_when_env_missing(monkeypatch: pytest.MonkeyPatch) -> None:
    """环境变量缺失时应回退到默认值。"""
    monkeypatch.delenv("COOPIE_TIMEOUT", raising=False)
    cfg = config.load_from_env()
...
from coopie import discover_configs
...
from coopie import parse_number
...
from coopie import StorageError, load
...
# 全量（含慢测试）
uv run pytest --cov=coopie --cov-branch --cov-report=term-missing

# HTML 报告（本地排查未覆盖行）
uv run pytest --cov=coopie --cov-report=html
```
...
def test_main_window_title(qtbot) -> None:
    ...
    assert window.windowTitle() == "coopie"
...
from coopie.widgets.search_box import SearchBox
